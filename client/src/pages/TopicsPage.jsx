...
export default function TopicsPage() {
  ...
  if (loading) return <LoadingSpinner label="Fetching topics..." />;
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-xs text-slate-400">Granular learning units tracked by your Digital Twin</p>
        </div>
        ...
        <Card title="Add New Topic">
          <form onSubmit={handleCreate} className="space-y-4">
            <div className="space-y-1.5">
              <label className="block text-xs font-semibold text-slate-300 uppercase tracking-wider">Select Subject</label>
              <select
            ...
            <div className="space-y-1.5">
              <label className="block text-xs font-semibold text-slate-300 uppercase tracking-wider">Difficulty Level (1 - 5)</label>
              <input type="range" min="1" max="5" value={formData.difficulty} onChange={(e) => setFormData({ ...formData, difficulty: Number(e.target.value) })} className="w-fullaccent-indigo-500" />
              <div className="text-right text-xs font-bold text-indigo-400">Level {formData.difficulty}</div>
            </div>
        ...
        {topics.map(t => (
          <Card key={t._id}>
            <div className="flex justify-between items-start">
              <span className="text-[10px] uppercase tracking-wider font-extrabold px-2.5 py-1 rounded-full bg-indigo-500/20 text-indigo-300 border border-indigo-500/30">
                {t.subjectId?.name || 'Subject'}
              </span>
              <span className="text-xs font-semibold text-slate-400">Diff: {t.difficulty}/5</span>
            </div>

            <h3 className="text-lg font-bold text-white mt-3">{t.name}</h3>
            <p className="text-xs text-slate-400 mt-1 line-clamp-2">{t.description || 'No description.'}</p>
          </Card>
        ))}
